  ...
  return (
    <>
      ...
      <div style={{ marginLeft: sidebarOpen ? "220px" : "0", padding: "2rem", width: "100%" }}>
        ...
        >
          <h2 style={{ marginBottom: "20px", color: "#1e293b" }}>My Tasks</h2>
          ...
          >
            <thead>
              <tr style={{ background: "#f8fafc", textAlign: "left" }}>
                ...
                <th style={{ padding: "12px 16px", borderBottom: "2px solid #e2e8f0" }}>Actions</th>
              </tr>
            </thead>
            <tbody>
              {tasks.length > 0 ? (
                tasks.map((task) => (
                  <tr key={task.id} style={{ borderBottom: "1px solid #e2e8f0" }}>
                    ...
                    <td style={{ padding: "12px 16px" }}>{task.assignedByName || "N/A"}</td>
                    <td style={{ padding: "12px 16px" }}>
                      {task.dueDate ? new Date(task.dueDate).toLocaleDateString() : "N/A"}
                    </td>
                    <td style={{ padding: "12px 16px" }}>
                      <Link
                        to={`/employee/update-task/${task.id}`}
